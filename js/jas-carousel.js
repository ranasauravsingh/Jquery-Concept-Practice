...
(function ($) {
	...
	$.fn.jasCarousel = function (option) {
		...
		let init = function () {
			...
			carousel.innerSlide = $(el)
				.children()
				.css({ transition: "all " + carousel.settings.speed + "ms" });
			...
			$("> *", carousel.settings.boxSelector).wrap(
				"<div class='" +
					carousel.settings.slideSelector.replace(".", "") +
					"'></div>"
			);
		...
		let setup = function () {
			...
			carousel.tz =
				Math.round(
					(carousel.settings.mode == "vertical"
						? $(el).height()
						: $(el).width()) /
						2 /
						Math.tan(Math.PI / carousel.slideCount)
				) + carousel.settings.margin;
			$(el).css({ perspective: carousel.settings.perspective + "px" });
			$(carousel.settings.boxSelector, el).css({
				transform: "translateZ(" + -carousel.tz + "px) rotateY(0deg)",
				transition: "all " + carousel.settings.speed + "ms",
			});
				...
				if (carousel.settings.mode == "vertical") {
					$(this).css({
						transform:
							...
							carousel.tz +
							"px) " +
							(carousel.settings.slideFrontFace
								? " rotateX(" + -newdeg + "deg)"
								: ""),
					});
				} else {
		...
		let appendControl = function () {
			carousel.controls.next = $(
				"<a href='#' class='next'>" + carousel.settings.nextText + "</a>"
			);
			carousel.controls.prev = $(
				"<a href='#' class='prev'>" + carousel.settings.prevText + "</a>"
			);
			...
			carousel.controls.navigationWrap
				.append(carousel.controls.next)
				.append(carousel.controls.prev);
		...
		el.goToSlide = function (direction = carousel.settings.autoDirection) {
			...
			carousel.settings.onSlideBefore.call(
				el,
				carousel.children.eq(carousel.currentSlideIndex),
				carousel.currentSlideIndex
			);
			...
			if (carousel.settings.mode == "vertical") {
				...
				carousel.settings.slideFrontFace
					? $(carousel.innerSlide, el).css({
							transform: "rotateX(" + -carousel.currentDeg + "deg)",
					  })
					: "";
			} else {
				$(carousel.settings.boxSelector, el).css({
				...
				$(carousel.settings.boxSelector, el).one(
					...
					function (e) {
						...
						carousel.children.eq(carousel.currentSlideIndex).addClass("active");
						carousel.settings.onSlideAfter.call(
							el,
							carousel.children.eq(carousel.currentSlideIndex),
							carousel.currentSlideIndex
						);
				...
				carousel.settings.slideFrontFace
					? $(carousel.innerSlide, el).css({
							transform: "rotateY(" + -carousel.currentDeg + "deg)",
					  })
					: "";
			}
		};
